...
def _get_local_superregion():
    """Get name of the superregion at Yelp where yelp_kafka instance is running (caller)
...
def get_region_cluster(cluster_type, client_id, region=None):
    """Get the kafka cluster for given region at Yelp. If no region is given, we default
    to local region.
...
def get_region_logs_stream(client_id, stream, region=None):
    """Get the kafka cluster logs for given region and stream at Yelp.
...
def get_region_logs_regex(client_id, regex, region=None):
    """Get the kafka cluster logs for given region and log-regex at Yelp.
    If no region is given, we default to local region.
...
def get_superregion_logs_stream(client_id, stream, superregion=None):
    """Get the kafka cluster logs for given superregion and stream at Yelp.
