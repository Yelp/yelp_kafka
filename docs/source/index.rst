...
can send metrics on latency and error counts by setting the `metrics_reporter`
config parameter to `"yelp_meteorite"`:
...
available in Yelp's fork of yelp-kafka (which yelp_kafka uses as a dependency).
